...
For managed/cloud deployments, [Trigger.dev](https://trigger.dev/) is also supported as a job provider. Set `NEXT_PRIVATE_JOBS_PROVIDER=trigger` and configure `TRIGGER_SECRET_KEY`.
