...
| Variable                     | Description                                                                            | Default |
| ---------------------------- | -------------------------------------------------------------------------------------- | ------- |
| `NEXT_PRIVATE_JOBS_PROVIDER` | Jobs provider: `local` (PostgreSQL), `bullmq` (Redis), or `trigger` (managed service) | `local` |
...
| `TRIGGER_API_URL`    | No       | Custom Trigger.dev API URL (self-hosted)     |
